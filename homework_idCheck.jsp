...
<script>
    function clickEvent() {
        ...
        if(<%=isCheck%> == true) {
            ...
            window.opener.document.getElementsByClassName("input-data")[0].value=""
            window.opener.document.getElementsByClassName("input-data")[0].focus()
            window.close()
        }else{
            alert("사용 가능한 아이디 입니다.")
            window.opener.document.getElementById("check-dul").value="true"
            window.opener.document.getElementsByClassName("input-data")[0].value = idTag.value
            window.close()     
        }
    }
