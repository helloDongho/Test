...
<%
    ...
    String sql = "SELECT username,userdepartment FROM user WHERE userid=? AND userpw=?";
    ...
    String depUserSql = "SELECT username,usernum FROM user WHERE userdepartment=?";
    PreparedStatement depUserQuery = connect.prepareStatement(depUserSql);
    depUserQuery.setString(1, data.get(0).get(1));

    ResultSet userResult = depUserQuery.executeQuery();
    ArrayList<ArrayList<String>> userNameData = new ArrayList<ArrayList<String>>(); 
    while(userResult.next()) {
        ArrayList<String> userNametmpData = new ArrayList<String>(); 
        userNametmpData.add(userResult.getString(1));
        userNametmpData.add(userResult.getString(2));
        userNameData.add(userNametmpData);
    }

    Boolean isUserLoad = false;
...
<head>
    ...
    <link rel="stylesheet" href="CalendarLeaderPage/Calendar_Leader_page.css" type="text/css">
...
<body>
    ...
    <script>
        ...
        function loadUser() {
            if(<%=isUserLoad%> == true) {
                var UserList = "<%=userNameData%>" // 이름이랑 usernum 같이 가져왔음
                ...
                if(depart == "develope") {
                    navHeaderTag.innerHTML = "개발팀"
                }
                else if(depart == "education") {
                    navHeaderTag.innerHTML = "교육팀"
                }
                else if(depart == "management") {
                    navHeaderTag.innerHTML = "운영팀"
                }
            }
            function accessUser() {
